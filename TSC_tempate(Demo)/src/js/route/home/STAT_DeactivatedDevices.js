...
class DeactivateDevice extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render () {      
        return (
            <Grid id='other_blank' fluid>
                <Row>
                    <Col md={12}>
                         <PageTitile text={`${'Sales Statistics'}/${'Deactivate Devices'}`} />
                    </Col>
                </Row>
                <Row>
                ...
                <Row>
                    <Col md={6}>
                        <StatDoughnut type='Model' statData={this.deaDevice} 
                ...
                <Row>                   
                    <Col md={12}>
                        ...
                        >
                            <div className='linegraphTitle'>Deactivate Devices</div>
                            <LineChart
    ...
    GetTitle () {
        ...
        return 'Total Deactivate Devices (' + title + ')';
    };
    ...
    doFetch () {
        ...
        result = DeactivateDevAPI.deactivatepie(this.formData);
        result.then((source) => {
            if (ServerAPI.errorCodeCheck(source)) 
            {
                this.deaDevice = source.data;    
            }       
        ...
        this.setState({ isDoughnutLoading: true });
        result = DeactivateDevAPI.deactivateset(this.formData);
        result.then((source) => {
            if (ServerAPI.errorCodeCheck(source)) 
            {
    ...
    resolveFetchData (source) {
        ...
        source.map((entry) => {
            ...
            this.deaDevice.push({val: total, model:entry.model});
        });
    };
    /*resolveFetchData (source) {
        this.lineData.rowlabels = [];
